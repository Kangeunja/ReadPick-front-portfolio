...
const MyPageEdit = ({ isDefaultImage, uploadedImage, handleEditProfileImg, userInfo, fetchUserInfo }: any) => {
  ...
  const [idEditMode, setIdEditMode] = useState<'default' | 'checking' | 'confirmed'>('default'); // 아이디 변경하기 버튼 클릭시 상태변화모드
  const [pwEditMode, setPwEditMode] = useState<'default' | 'checking'>('default'); // 비밀번호 변경하기 버튼 클릭시 현재비밀번호, 새비밀번호 상태
  ...
  const handlePwChange = (field: string, value: string) => {
    ...
    if (field === 'newPw') {
      ...
      else if (value === passwordForm.currentPw) msg = '현재 비밀번호와 다르게 입력해주세요.';
      else if (!pwRegex.test(value)) msg = '영문, 숫자, 특수문자를 포함해주세요.';
    }
  ...
  // 최종 유효성 검사
  const validateForm = () => {
    if (!editInfo.nickName) return { field: 'nickName', message: '닉네임을 입력해주세요.' };

    if (!editInfo.userName) return { field: 'userName', message: '이름을 입력해주세요.' };

    if (!editInfo.id) return { field: 'id', message: '아이디를 입력해주세요.' };

    if (!editInfo.email) return { field: 'id', message: '이메일을 입력해주세요.' };

    if (pwEditMode === 'checking') {
      if (!passwordForm.currentPw) return { field: 'currentPw', message: '현재 비밀번호를 입력해주세요.' };
      if (!passwordValidation.newPwMessage) return { field: 'newPw', message: '새 비밀번호를 입력해주세요.' };
      if (!passwordValidation.isConfirmPwValid) return { field: 'confirmPw', message: '비밀번호가 일치하지 않습니다.' };
    }
  };
    ...
    <>
      <div className="mx-auto mb-[200px] flex w-main-w flex-col items-center pt-[50px]">
        <div className="flex w-[100px] justify-center">
          <div
            className={`mb-[23px] flex h-20 w-20 items-center justify-center rounded-[50px] ${
              isDefaultImage ? 'border border-[#1b1b1b]' : ''
            }`}
          >
            {isDefaultImage ? (
              <div className="bg-icon-default h-[30px] w-[30px] bg-cover" />
            ) : (
              <img src={uploadedImage || userInfo.fileName} alt="프로필 이미지" className="h-20 w-20 rounded-[50px]" />
            )}
          </div>
        </div>
        <div className="mb-[60px] flex w-[100px] justify-between">
          <div onClick={() => editImgRef.current?.click()}>
            <button className="h-[30px] w-[61px] cursor-pointer rounded-[10px] border-none bg-[#a0a0a0] text-xs text-white hover:bg-[#1e7373]">
              {isDefaultImage ? '사진추가' : '사진수정'}
            </button>
            <input type="file" ref={editImgRef} accept="image/*" style={{ display: 'none' }} onChange={handleEditProfileImg} />
          </div>

          <button
            className={`h-[30px] w-[30px] rounded-[10px] border-none bg-[#a0a0a0] bg-trash-icon bg-[length:15px_15px] bg-center bg-no-repeat text-xs text-white ${isDefaultImage ? 'cursor-not-allowed opacity-[0.3]' : 'hover:bg-[#1e7373]'}`}
        ...
        <FormField
          ...
          rightSlot={
            idEditMode === 'default' ? (
              <button className="mypage-edit__id-edit" onClick={() => setIdEditMode('checking')}>
                변경하기
              </button>
            ) : (
              <div className="mypage-edit__id-btn-group">
                <button className="mypage-edit__id-check" onClick={handleIdCheck}>
                  중복확인
                </button>
                <button
          ...
          messageSlot={
            idCheckMessage && (
              <p className={`mypage-edit__id-check-message ${idValidation.isValid ? 'success' : 'error'}`}>{idCheckMessage}</p>
            )
          }
        />
        ...
        {pwEditMode === 'default' && (
          <FormField
            ...
            rightSlot={
              <button className="mypage-edit__pw-edit" onClick={() => setPwEditMode('checking')}>
                변경하기
              </button>
            }
          />
          ...
          <>
            <FormField
              ...
              rightSlot={
                ...
                >
                  {passwordForm.currentPw ? <VisibilityIcon /> : <VisibilityOffIcon />}
                </span>
              }
            />
            ...
            <FormField
              ...
              rightSlot={
                <span className="mypage-edit__visible" onClick={() => setIsNewPwVisible((prev) => !prev)}>
                  {isNewPwVisible ? <VisibilityIcon /> : <VisibilityOffIcon />}
                </span>
              }
              messageSlot={
                passwordValidation.newPwMessage && (
                  <p className={`mypage-edit__newPw-message ${passwordValidation.isNewPwValid ? 'success' : 'error'}`}>
                    {passwordValidation.newPwMessage}
                  </p>
                )
              }
            ...
            <FormField
              ...
              rightSlot={
                <span className="mypage-edit__visible" onClick={() => setIsConfirmVisible((prev) => !prev)}>
                  {isConfirmPwVisible ? <VisibilityIcon /> : <VisibilityOffIcon />}
                </span>
              }
              messageSlot={
                passwordValidation.confirmPwMessage && (
                  <p className={`mypage-edit__confirmPw-message ${passwordValidation.isConfirmPwValid ? 'success' : 'error'}`}>
                    {passwordValidation.confirmPwMessage}
                  </p>
                )
              }
            ...
            <div className="relative mb-[45px] flex w-[363px] flex-col">
              <button type="button" className="mypage-edit__pw-cancel" onClick={() => setPwEditMode('default')}>
                비밀번호 변경 취소
              </button>
            </div>
          </>
      ...
      {successMessage && <MessagePopup message={successMessage} onFinish={() => setSuccessMessage(null)} />}
