...
const BookCard = ({ item, imgUrl, onCardClick }: BookCardProps) => {
  ...
  return (
    <div className="w-[160px] cursor-pointer" onClick={() => onCardClick(item.bookIdx)} {...hoverProps}>
      <img
        className={`border-borderLightColor mb-[15px] h-[200px] w-[160px] border object-cover transition-all duration-[0.3s] ease-in-out ${isHovered ? '-translate-y-1 shadow-[3px_2px_9.6px_1px_rgba(193,193,193,1)]' : 'translate-y-0 shadow-none'} `}
