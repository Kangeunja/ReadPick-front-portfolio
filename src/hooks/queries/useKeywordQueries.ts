...
import { getKeywordList, getMainKeywordList, searchBooks, fetchBooksByBsIdx, fetchBooksByBssIdx } from 'api/searchApi';
...
// 키워드/검색 기반 책 목록 조회
export const useKeywordBooksQuery = ({ bsIdx, bssIdx, option, keywordText }: KeywordParams) => {
  return useQuery({
    queryKey: ['keywordBooks', { bsIdx, bssIdx, option, keywordText }],
    queryFn: async () => {
