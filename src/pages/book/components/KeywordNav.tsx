...
const KeywordNav = ({ keywordToggle, keywordList, selectedBssIdx, onBsClick, onBssClick, onToggleClick }: BsItemProps) => {
  return (
    <div className="float-left mb-[150px] box-border w-[260px] overflow-hidden border border-borderGrayColor bg-[#f9f9f9] p-[30px]">
      {keywordList.map((item) => (
        <div key={item.bsIdx}>
          <div
            className="mb-[5px] cursor-pointer break-keep font-bold hover:underline"
            onClick={() => {
